...
class EventDataManagerConfig(VanillaDataManagerConfig):
    ...
    is_colored = True
    #event_threshod=0.10  # for real data
    event_threshod = 0.5 # 0.25 # for nerf data
    polarity_offset = 0
    #neg_ratio=0.05
...
class EventDataManager(VanillaDataManager):
    ...
    def sample1(self, batch):
        ...
        batch["event_frame_selected"] = (event_frame_selected + self.config.polarity_offset) * self.config.event_threshod
        return ray_bundle, batch
    ...
    def sample0(self, batch):
        ...
        if True:
            ...
            #CONSOLE.print("ray_indices", ray_indices[:10])
            #CONSOLE.print("cam_count", self.train_ray_generator.cameras.size)
            #CONSOLE.print("ray_bundle", ray_bundle.shape)
        else:
            nonzero_indices = torch.nonzero(event_frame)
            #print(nonzero_indices.shape)
            #if nonzero_indices.shape[0] > 4096:
            #    nonzero_indices = nonzero_indices[:4096]
            #coords = nonzero_indices[:, :2]
            p_batch_size = nonzero_indices.shape[0]
            n_batch_size = p_batch_size // 10
            zero_indices = torch.nonzero(event_frame.sum(2) == 0)
            if p_batch_size > nonzero_indices.shape[0]:
                p_batch_size = nonzero_indices.shape[0]
            if n_batch_size > zero_indices.shape[0]:
            ...
            ones = torch.ones((coords.shape[0], 1))
            #print (ones.shape)
            ray_indices = torch.concat((torch.concat((ones * split[0], coords), dim=-1), 
                                        torch.concat((ones * split[1], coords), dim=-1)), dim=0).int()
            #print("ray_indices", ray_indices.shape)
            ray_bundle = self.train_ray_generator(ray_indices)
            batch["acc_map_selected"] = event_frame[coords[:, 0], coords[:, 1]]
        ray_bundle : RayBundle = self.train_ray_generator(ray_indices)
